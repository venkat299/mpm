...
urlpatterns = [
    ...
    url(r'^emp/edit/(?P<pk>\w+)/$', views.EmployeeUpdate.as_view(), name='emp_edit'),
    # url(r'^delete/(?P<pk>\w+)/$', views.bug_delete, name='bug_delete'),

    url(r'^emp/add/$', views_transaction.AddEmpView.as_view(),name="emp_add"),
    url(r'^emp/update/(?P<pk>\w+)/$', views_transaction.EditEmpView.as_view(), name='emp_update'), 
    url(r'^emp/transfer/(?P<pk>\w+)/$', views_transaction.EditTransferView.as_view(), name='emp_transfer'),   
    url(r'^emp/promote/(?P<pk>\w+)/$', views_transaction.EditPromoView.as_view(), name='emp_promote'),   
    url(r'^emp/terminate/(?P<pk>\w+)/$', views_transaction.EditTerminateView.as_view(), name='emp_terminate'), 
    url(r'^emp/detail/(?P<pk>\w+)/$', views_transaction.employee_detail, name='emp_detail'),     
